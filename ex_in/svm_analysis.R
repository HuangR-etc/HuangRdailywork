# 主分析脚本：系统发育树数据模拟与建模分析流程 - SVM版本
# 根据流程图实现完整的分析流程，使用支持向量机（SVM）作为核心模型
setwd("/home/huangr/projects/2026_03_ex_vs_in")
# 加载必要的R包
cat("加载必要的R包...\n")

# 核心包 - 必须的
required_packages <- c("ape", "phytools", "PVR", "e1071")  # 将randomForest替换为e1071用于SVM
...
# 可选包 - 如果加载失败，跳过相关功能
optional_packages <- c("caret", "glmnet", "ggplot2", "dplyr", "tidyr", "kernlab")
...
kernlab_available <- require("kernlab", quietly = TRUE)
...
# 7. 超参数调优函数（SVM版本） ------------------------------------------------------------
tune_svm_model <- function(X, y, cv_folds, param_grid) {
  best_rmse <- Inf
  best_params <- NULL
...
tune_result1 <- tune_svm_model(X_train, y_train, cv_folds1, svm_param_grid)
cat("    最佳参数: kernel=", tune_result1$best_params$kernel, 
    ", cost=", tune_result1$best_params$cost, 
    ", gamma=", tune_result1$best_params$gamma,
    ", epsilon=", tune_result1$best_params$epsilon, 
    ", RMSE=", round(tune_result1$best_rmse, 4), "\n")

# 策略2: 隐形利用系统发育（随机CV）
cat("  策略2: 隐形利用系统发育（随机CV）...\n")
cv_folds2 <- strategy2_cv(X_train, y_train, nfolds = 5)
tune_result2 <- tune_svm_model(X_train, y_train, cv_folds2, svm_param_grid)
cat("    最佳参数: kernel=", tune_result2$best_params$kernel, 
    ", cost=", tune_result2$best_params$cost, 
    ", gamma=", tune_result2$best_params$gamma,
    ", epsilon=", tune_result2$best_params$epsilon, 
...
tune_result4 <- tune_svm_model(X_train, y_train, cv_folds4, svm_param_grid)
cat("    最佳参数: kernel=", tune_result4$best_params$kernel, 
    ", cost=", tune_result4$best_params$cost, 
    ", gamma=", tune_result4$best_params$gamma,
    ", epsilon=", tune_result4$best_params$epsilon, 
...
model1 <- svm(
  ...
  y = y_train,
  type = "eps-regression",
  kernel = tune_result1$best_params$kernel,
  cost = tune_result1$best_params$cost,
  gamma = tune_result1$best_params$gamma,
  epsilon = tune_result1$best_params$epsilon
)
...
model2 <- svm(
  ...
  y = y_train,
  type = "eps-regression",
  kernel = tune_result2$best_params$kernel,
  cost = tune_result2$best_params$cost,
  gamma = tune_result2$best_params$gamma,
  epsilon = tune_result2$best_params$epsilon
)
...
model4 <- svm(
  ...
  y = y_train,
  type = "eps-regression",
  kernel = tune_result4$best_params$kernel,
  cost = tune_result4$best_params$cost,
  gamma = tune_result4$best_params$gamma,
  epsilon = tune_result4$best_params$epsilon
)
...
if (ggplot2_available) {
  ...
  p1 <- ggplot(results_df, aes(x = Clade, y = RMSE, fill = Model)) +
    ...
    labs(title = "SVM Prediction Performance by Clade (RMSE)",
         x = "Clade", y = "RMSE") +
    theme_minimal() +
  ...
  p2 <- ggplot(overall_results, aes(x = Model, y = Overall_RMSE, fill = Model)) +
    ...
    labs(title = "SVM Overall Prediction Performance (RMSE)",
...
# 保存模型对象
saveRDS(model1, file = "svm_model1_controlled_phylogeny.rds")
saveRDS(model2, file = "svm_model2_hidden_phylogeny.rds")
saveRDS(model3, file = "svm_model3_enhanced_phylogeny.rds")
saveRDS(model4, file = "svm_model4_block_cross_validation.rds")
...
saveRDS(results, file = "svm_analysis_results.rds")

# 保存可视化图形
if (ggplot2_available) {
  ggsave("svm_clade_performance_comparison.png", p1, width = 10, height = 6)
  ggsave("svm_overall_performance_comparison.png", p2, width = 8, height = 6)
  cat("  - svm_clade_performance_comparison.png: SVM类群性能比较图\n")
  cat("  - svm_overall_performance_comparison.png: SVM总体性能比较图\n")
}

cat("SVM分析完成！结果已保存到文件。\n")
cat("生成的文件:\n")
cat("  - svm_model1_controlled_phylogeny.rds: SVM模型1对象\n")
cat("  - svm_model2_hidden_phylogeny.rds: SVM模型2对象\n")
cat("  - svm_model3_enhanced_phylogeny.rds: SVM模型3对象\n")
cat("  - svm_model4_block_cross_validation.rds: SVM模型4对象\n")
cat("  - svm_analysis_results.rds: 完整SVM分析结果\n")
cat("  - svm_clade_performance_comparison.png: SVM类群性能比较图\n")
cat("  - svm_overall_performance_comparison.png: SVM总体性能比较图\n")

...
#--------优化后的绘图--------
cluster_colors <- c("#E0795f", "#649cc9", "#a5cc5b", "#8877b6")
biased_color <- "#c72228"
...
# oma: 外边界边距 (下, 左, 上, 右)
# mfrow: 图形布局 (行数, 列数)
par(mar = c(3, 1, 3, 1),  # 增加底部边距为图例留出空间
    oma = c(0, 0, 2, 0), 
    mfrow = c(1, 2))

# 图1: PAM聚类结果
# plot.phylo参数:
# show.tip.label = FALSE: 不显示叶尖标签
# cex.main = 1.3: 标题字体大小
# font.main = 2: 标题字体粗细 (2=粗体)
# edge.width = 3: 树枝粗细
# edge.color = "gray30": 树枝颜色
plot(tree, 
...
# 添加叶尖标签颜色
# pch = 16: 实心圆点
# col = alpha(tip_colors, 0.8): 颜色带透明度
# cex = 1.2: 点的大小
tip_colors <- cluster_colors[folds[tree$tip.label]]
tiplabels(pch = 16, 
          col = alpha(tip_colors, 0.8), 
          cex = 1.2)

# 优化图例：单列排列，调整宽度
# "bottomleft": 图例位置在左下角
# bty = "o": 图例带边框
# bg = alpha("white", 0.8): 半透明白色背景
# box.col = "gray50": 边框颜色
# ncol = 1: 单列排列
# pt.cex = 1.5: 图例中点的大小
# cex = 1.1: 图例文字大小
# x.intersp = 0.8: 水平间距（减小以缩小宽度）
# y.intersp = 1.2: 垂直间距
# inset = c(0.02, 0.02): 距离边界的偏移量
legend("bottomleft", 
...
plot(tree, 
     show.tip.label = FALSE, 
     main = paste("PH-UC1: Cluster", biased_cluster, "with Bias"),
     cex.main = 1.3, 
     font.main = 2,
     edge.width = 3,
     edge.color = "gray30")
...
# 着色偏差类群
biased_tip_colors <- ifelse(folds[tree$tip.label] == biased_cluster, 
                            biased_color, neutral_color)
tiplabels(pch = 16, 
          col = alpha(biased_tip_colors, 0.8), 
          cex = 1.2)

# 优化图例：单列排列，调整宽度
legend("bottomleft", 
       legend = c("Biased cluster", "Other clusters"),
       col = c(biased_color, neutral_color), 
       pch = 16, 
       cex = 1.1, 
       bty = "o", 
       bg = alpha("white", 0.8),
...
# font = 2: 粗体
# line = 0.2: 距离图形顶部的行数
mtext("Phylogenetic Clustering and Bias Pattern Visualization", 
      outer = TRUE, 
      cex = 1.4, 
      font = 2, 
      line = 0.2)
...
#--------场景4可视化--------
ingroup_color <- "#8877b6"    # 内类群颜色（紫色系）
outgroup_color <- "#E0795f"   # 外类群颜色（橙色系）
failure_color <- "#c72228"    # 预测失败颜色（红色）
accurate_color <- "#649cc9"   # 预测准确颜色（蓝色）
...
# 添加叶尖标签颜色
tip_colors_uc4 <- ifelse(outgroup_cut == outgroup_id, outgroup_color, ingroup_color)
tiplabels(pch = 16, 
          col = alpha(tip_colors_uc4, 0.8), 
          cex = 1.2)
...
# inset = c(0.02, 0.02): 距离边界的偏移量
legend("bottomleft", 
       legend = c("Outgroup", "Ingroup"),
       col = c(outgroup_color, ingroup_color), 
       pch = 16, 
       cex = 1.1, 
       bty = "o", 
       bg = alpha("white", 0.8), 
...
plot(tree, 
     show.tip.label = FALSE, 
     main = paste("UC4: Outgroup Prediction Failure"),
     cex.main = 1.3, 
     font.main = 2,
     edge.width = 3,
     edge.color = "gray30")
...
# 着色预测失败类群
tip_colors_fail <- ifelse(outgroup_cut == outgroup_id, failure_color, accurate_color)
tiplabels(pch = 16, 
          col = alpha(tip_colors_fail, 0.8), 
          cex = 1.2)

# 优化图例：单列排列，调整宽度
legend("bottomleft", 
       legend = c("Prediction failure", "Accurate prediction"),
       col = c(failure_color, accurate_color), 
       pch = 16, 
       cex = 1.1, 
       bty = "o", 
       bg = alpha("white", 0.8),
...
# font = 2: 粗体
# line = 0.2: 距离图形顶部的行数
mtext("Scenario 4: Outgroup Prediction Failure Visualization", 
      outer = TRUE, 
      cex = 1.4, 
      font = 2, 
      line = 0.2)

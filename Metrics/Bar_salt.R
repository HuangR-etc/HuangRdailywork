...
library(nlme)
library(rr2)
setwd("D:/pine/paper_redo/2025_09_obs_vs_pred/get_obs_pred/salt")
#每次分析读取特定数据文件
filename <- "Barnum2024_salinity_clean.csv"
#------通用数据读取和分析代码----------
...
calculate_all_r2 <- function(final_data, tree_subset, comp_data, 
                             response_var = "reported_salinity_optimum", 
                             predictor_var = "salinity_optimum") {
  ...
  phylo_r2 <- calculate_phylo_r2(
    y = final_data$reported_salinity_optimum,
    yhat = final_data$salinity_optimum,
...
perform_random_split_analysis <- function(final_data, tree, n_iterations = 50, train_ratio = 0.8) {
  ...
  while (successful_iterations < n_iterations && iteration_count < n_iterations * 2) {
    ...
    if (result$success) {
      ...
      cat("第", successful_iterations, "次成功划分完成，训练集大小:", n_train, "测试集大小:", n_total - n_train, "\n")
    }
  }
...
cat("开始100次随机划分分析（允许失败重试）...\n")
random_results <- perform_random_split_analysis(final_data, tree, n_iterations = 100)

cat("所有分析完成！成功完成", nrow(random_results), "次迭代\n")
cat("结果已保存至:", random_split_file, "\n")
#--------划分2：筛选出原训练集，进行划分-----------
library(dplyr)
train_data <- final_data %>%
  filter(partition_salinity == "train")
...
test_data <- final_data %>%
  filter(partition_salinity == "test")

...
#### 1. Processing data ####
raw_data <- read.table("./data/full_species_table",header = T)
raw_tree <- read.tree("./data/full_species_raxml_tree.ph")

raw_data <- raw_data %>%
  mutate(tips_name = str_replace(species, "^([A-Z])", tolower))


first_match <- sapply(raw_tree$tip.label, function(b) {
  res <- grep(b, raw_data$tips_name, fixed = TRUE)
  ifelse(length(res) > 0, res[1], NA)
})


raw_data$tips_name[first_match]
raw_tree$tip.label

datatotree_df <- data.frame(tips_data = raw_data$tips_name[first_match], tips_tree = raw_tree$tip.label)

new_df <- merge(datatotree_df,raw_data, by.x="tips_data", by.y="tips_name", all.x=TRUE)

union(new_df$tips_tree,raw_tree$tip.label)




new_df <- new_df %>%
  filter(species != "Octopus_bimaculoides" & species != "Lottia_gigantea" & 
           species != "Crassostrea_gigas" & 
           species != "Tetranychus_urticae" & species != "Sarcoptes_scabiei" & 
           species != "Ixodes_scapularis" & species != "Stegodyphus_mimosarum" & 
           species != "Nasonia_vitripennis" & species != "Atta_cephalotes" & 
           species != "Solenopsis_invicta" & species != "Apis_mellifera" & 
           species != "Bombus_impatiens"
  )

my_tree <- keep.tip(raw_tree, tip = new_df$tips_tree)


saveRDS(my_tree,"./results/my_tree.rds")

##### 1.2 log_transformation ####
new_df$log_GS <- log(new_df$genome_size)
new_df$log_PS <- log(new_df$propagule_size)
new_df$Pd

write.csv(new_df,"results/example2.csv")

rownames(new_df) <- new_df$tips_tree
